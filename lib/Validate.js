...
module.exports = class Validate {
  ...
  parse(req, res) {
    if (!req.query.url || !req.query.url.length) {
      res.status(404).send('need a valid url');
      return false;
    }
    let result;
    try {
      result = url.parse(req.query.url);
    } catch(e) {
      res.status(404).send('need a valid url');
      return false;
    }
    ...
    try {
      result = JSON.parse(req.query.opt);
    } catch(e) {
      res.status(400).send('Invalid params - use JSON encoded string for opt');
      return false;
    }
    if (!this.validate(result)) {
      res.status(400).send(this.validate.errors);
      return false;
    }
